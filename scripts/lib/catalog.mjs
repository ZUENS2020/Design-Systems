...
<script>
  ...
  function renderStats(items) {
    const themes = new Set(items.flatMap((item) => item.themes));
    const tags = new Set(items.flatMap((item) => item.tags));
    document.querySelector('#stats').innerHTML = [['Systems', items.length], ['Themes', themes.size], ['Tags', tags.size]]
  ...
  function renderButtons(selector, values, selected, key) {
    document.querySelector(selector).innerHTML = ['all', ...values].map((value) => {
      ...
      return '<button class="pill' + active + '" data-key="' + key + '" data-value="' + value + '">' + esc(label) + '</button>';
    }).join('');
  }
...
function renderDetailPage(system, site) {
  ...
  <footer class="footer">This wrapper is generated from sidecar metadata. The design system HTML is untouched.</footer>
</div>`
  });
}
